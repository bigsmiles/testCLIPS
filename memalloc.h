...
/*
#define get_struct_help(theEnv,size) \
	((MemoryData(theEnv)->MemoryTable[size] == NULL) ? \
	((void *) genalloc(theEnv, size)) : \
...
#define get_struct(theEnv,type) ((struct type*) my_get_struct(theEnv,sizeof(struct type)))
*/
...
/*
#define get_var_struct_help(theEnv,size,vsize) \
	((((size + vsize) <  MEM_TABLE_SIZE) ? \
	(MemoryData(theEnv)->MemoryTable[size + vsize] == NULL) : 1) ? \
	((void *) genalloc(theEnv, (size + vsize))) : \
	((MemoryData(theEnv)->TempMemoryPtr = MemoryData(theEnv)->MemoryTable[size + vsize]), \
	MemoryData(theEnv)->MemoryTable[size + vsize] = MemoryData(theEnv)->TempMemoryPtr->next, \
	((void *) MemoryData(theEnv)->TempMemoryPtr)))

#define get_var_struct (theEnv,types,vsize) ((struct types *) my_get_var_struct(theEnv,sizeof(struct types),vsize))
*/
...
#define SetOutOfMemoryFunction(a) EnvSetOutOfMemoryFunction(GetCurrentEnvironment(),a)
   ...
   //LOCALE void							 *my_get_struct(void *, size_t);

   //LOCALE void							 *my_get_var_struct(void *, size_t,long);
   LOCALE void							 *my_MemoryData(void *, size_t);
