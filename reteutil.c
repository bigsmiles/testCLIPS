...
#if !OPTIMIZE
   struct fact* cur_fact = (struct fact*) theEntity;
   if (cur_fact->alphaMatch != NULL){
	   return (struct partialMatch*)cur_fact->alphaMatch;
   }
   printf(" %s alphaMatch\n",cur_fact->whichDeftemplate->header.name->contents);
#else
   struct alphaMemoryHash *tryAlpha;
   struct partialMatch *p;
   hashValue = AlphaMemoryHashValue(theHeader, hashOffset);
   tryAlpha = FindAlphaMemory(theEnv, theHeader, hashValue);
   if (tryAlpha != NULL && tryAlpha->alphaMemory != NULL)
   {
	   if (tryAlpha->alphaMemory->binds[0].gm.theMatch->matchingItem == (struct patternEntity *) theEntity)
		   return tryAlpha->alphaMemory;
   }
   if (tryAlpha != NULL && tryAlpha->endOfQueue != NULL)
   {
	   p = tryAlpha->endOfQueue;
	   while (p != NULL){
		   if (p->binds[0].gm.theMatch->matchingItem == (struct patternEntity *) theEntity)
			   return p;
		   p = p->prevInMemory;
	   }
   }
