...
globle void *CreateEnvironmentDriver(
  ...
  struct externalAddressHashNode **externalAddressTable)
  {
   ...
   if (++flag == 1)
   {
	   activeNodeHead = (struct activeJoinNode*) malloc(sizeof(struct activeJoinNode));
	   activeNodeTail = NULL;
	   activeNodeHead->currentJoinNode = NULL;
   ...
   if (++flag == 2){
	   ...
	   joinNodeListTail = joinNodeListHead;
	   joinNodeListHead->join = NULL;
	   joinNodeListHead->next = NULL;
	   joinNodeListHead->pre = NULL;
   }
#endif
